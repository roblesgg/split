/* ============================================================
   split — el objetivo de gasto de una cuenta

   Cuánto quieres gastar como mucho DESDE UNA CUENTA, que se vacía solo
   cuando cierra el ciclo. Es otro eje que los límites del mes: aquellos
   miran categorías, este mira de dónde sale el dinero. Por eso vive
   aparte y se llama distinto en el código, igual que en la pantalla. No es un tope duro: la app avisa, no bloquea. Bloquear haría
...
(function () {
  ...
  function setObjetivo(accId, importe) {
    var a = cuenta(accId);
    if (!a) return null;
  ...
  function estadoDeObjetivo(accId, key) {
    var limite = objetivoDe(accId);
  ...
  function cuentasConObjetivo() {
    return D.state.accounts.filter(function (a) { return objetivoDe(a.id) != null; });
  }

  /* --- lo que se lleva el espacio común --- */
  D.cuentasConObjetivo = cuentasConObjetivo;
  D.estadoDeObjetivo = estadoDeObjetivo;
  D.gastoDeCuenta = gastoDeCuenta;
  D.objetivoDe = objetivoDe;
  D.setObjetivo = setObjetivo;
})();
